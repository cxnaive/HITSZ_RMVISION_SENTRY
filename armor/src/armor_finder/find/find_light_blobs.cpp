...
//            blue_cnt += roi.at<cv::Vec3b>(row, col)[0];
            red_cnt += roi.ptr<cv::Vec3b>(row)[col][2];
            blue_cnt += roi.ptr<cv::Vec3b>(row)[col][0];
...
bool ArmorFinder::findLightBlobs(const cv::Mat &src, LightBlobs &light_blobs) {
    ...
    cv::threshold(color_channel, src_bin_dim, 225, 255, cv::THRESH_BINARY); // 二值化对应通道
...
// 同时剔除重复的灯条，剔除冗余计算，即对两次找出来的灯条取交集。
    std::vector<std::vector<cv::Point>> light_contours_light, light_contours_dim;      //vector<std::vector<cv::Point>>表示元素为轮廓的数组
    ...
    for (int i = 0; i < light_contours_light.size(); i++) {
        if (hierarchy_light[i][2] == -1) {                      //如果第二个轮廓没有对应的后一个轮廓、前一个轮廓、父轮廓和内嵌轮廓，则相应的hierarchy[i][*]被置为-1
            cv::RotatedRect rect = cv::minAreaRect(light_contours_light[i]);
            if (isValidLightBlob(light_contours_light[i], rect)) {             //如果是一个灯条
